...
static void SendTxData(void)
{
 ...
 // uint16_t humidity = 0; //added by ons
  ...
  temperature = (SYS_GetTemperatureLevel() >> 8);
   ...
   temperature   = (uint16_t)(sensor_data.temperature);      //// added

  pressure    = (uint16_t)(sensor_data.pressure * 100 / 10);      /* in hPa / 10 */

 humidity    = (uint16_t)(sensor_data.humidity);    /*added by ons */

  AppData.Port = LORAWAN_USER_APP_PORT;

	/* Display data on the terminal*/

sprintf((char*)buffer_ons,"%u temperature : degree C \r\n",
		    	  	              ((unsigned int)temperature));
  vcom_Trace(buffer_ons,strlen((char*)buffer_ons));

  sprintf((char*)buffer_ons,"%u Humidity \r\n",
 		    	  	              ((unsigned int)humidity));
   vcom_Trace(buffer_ons,strlen((char*)buffer_ons));


  sprintf((char*)buffer_ons,"%u pressure : hpa \r\n",
